...
function rgbToHsl([r, g, b]) {
  r /= 255; g /= 255; b /= 255; const mx = Math.max(r, g, b), mn = Math.min(r, g, b), l = (mx + mn) / 2; if (mx === mn) return [0, 0, l];
...
function derivePalette(pal, { hue = 0, sat = 1, light = 0, paper, ink, finish, night } = {}) {
  ...
  return {
    ...pal, paper: paper || pal.paper, ink: ink || pal.ink, night: night || pal.night, finish: finish || pal.finish,
...
function duotone(inkA, inkB, paper = '#f0ece2', finish = 'riso') {
  return makePalette({
    paper, ink: inkA, night: shade(inkA, .6), chalk: paper, chalkDim: mix(inkA, paper, .5), guide: alpha(inkA, .5),
...
function key(t, K, e = easeIO) {
  const vals = k => k.filter(v => typeof v === 'number').slice(1), one = vals(K[0]).length === 1, out = v => one ? v[0] : v; if (t <= K[0][0]) return out(vals(K[0]));
...
function keyPath(t, K, o = {}) {
  const { ease = easeInOutSine } = o; const n = K.length, P = i => K[clamp(i, 0, n - 1)].slice(1); if (n < 3) return [].concat(key(t, K, ease));
...
function blob(cx, cy, rx, ry, seed = 1, o = {}) {
  const { amp = .05, rot = 0, n = 48 } = o; const r = rng(seed), k1 = 2 + (r() * 2 | 0), k2 = k1 + 1 + (r() * 2 | 0), p1 = r() * TAU, p2 = r() * TAU, p3 = r() * TAU, p = [];
...
function smoothPts(pts, close = false, step = 4, corner = .8) {
  const n = pts.length; if (n < 2) return pts.map(p => p.slice()); const P = i => close ? pts[((i % n) + n) % n] : pts[clamp(i, 0, n - 1)];
  ...
  for (let i = 0; i < segs; i++) {
    const p1 = P(i), p2 = P(i + 1), c1 = sharp[i % n], c2 = sharp[(i + 1) % n], p0 = c1 ? [2 * p1[0] - p2[0], 2 * p1[1] - p2[1]] : P(i - 1), p3 = c2 ? [2 * p2[0] - p1[0], 2 * p2[1] - p1[1]] : P(i + 2), m = Math.max(1, Math.round(Math.hypot(p2[0] - p1[0], p2[1] - p1[1]) / step));
...
function warp(pts, seed = 1, amp = 4, close = true, step = 24) {
  const n = pts.length, out = [], segs = close ? n : n - 1; for (let i = 0; i < segs; i++) { const a = pts[i], b = pts[(i + 1) % n], m = Math.max(1, Math.round(Math.hypot(b[0] - a[0], b[1] - a[1]) / step)); for (let k = 0; k < m; k++) out.push([lerp(a[0], b[0], k / m), lerp(a[1], b[1], k / m)]); } if (!close) out.push(pts[n - 1].slice());
...
function camKeys(c, tau, K, o = {}) {
  const { ease = easeInOutSine, hand = 0, seed = 9 } = o; const v = keyPath(tau, K, { ease }), z = v[2] ?? 1, rot = v[3] ?? 0;
...
function wob(c, pts, amp, seed, close = false, o = {}) {
  const { smooth = true, step = 4, corner = .8, pressure = 0, freq = 1 } = o; if (pts.length < 2) return;
...
function hatch(c, path, box, o = {}) {
  ...
  for (let v = -R; v <= R; v += gap) for (let u = -R; u <= R; u += len * 1.7) {
    const uu = u + (r() - .5) * jitter * 2, L = len * (.5 + r() * 1.0);
...
function scribble(c, path, cx, cy, o = {}) {
  const { colors = PAL.accents, amp = 5, alpha: al = .55, width = 1.1, seed = 3 } = o; const r = rng(seed); c.save(); c.globalAlpha = al; c.lineWidth = width;
...
function construction(c, cx, cy, R, seed, color = PAL.guide, al = 1) {
  const r = rng(seed); c.save(); c.globalAlpha = al; c.strokeStyle = color; c.lineWidth = .9;
  for (let i = 0; i < 3; i++) {
    const a = r() * Math.PI, L = R * (1.3 + r() * 1.2); c.beginPath(); c.moveTo(cx - Math.cos(a) * L, cy - Math.sin(a) * L); c.lineTo(cx + Math.cos(a) * L, cy + Math.sin(a) * L); c.stroke();
...
function squiggleText(c, x, y, w, lines, o = {}) {
  const { color = PAL.ink, seed = 1, lineH = 16, amp = 4, width = 1.3, gap = .4 } = o; const r = rng(seed); c.save(); c.strokeStyle = color; c.lineWidth = width; c.lineCap = 'round'; c.lineJoin = 'round';
  for (let l = 0; l < lines; l++) {
    let px = x; const py = y + l * lineH, end = x + w * (l === lines - 1 ? .35 + r() * .5 : .9 + r() * .1);
...
function handText(c, text, x, y, o = {}) {
  const { size = 64, ink = PAL.ink, ink2 = PAL.accents[0], offset = 3, align = 'left' } = o; c.save(); c.font = `${size}px ${HAND_FONT}`; c.textAlign = align; c.textBaseline = 'alphabetic';
...
function dotScreen(c, path, box, o = {}) {
  ...
  for (let v = -R; v <= R; v += cell) for (let u = -R; u <= R; u += cell) {
    const x = cx + ca * u - sa * v + (r() - .5) * jitter * cell, y = cy + sa * u + ca * v + (r() - .5) * jitter * cell;
...
function printPlate(c, src, o = {}) {
  ...
  for (let v = -R; v <= R; v += cell) for (let u = -R; u <= R; u += cell) {
    const x = W / 2 + ca * u - sa * v + (r() - .5) * jitter * cell, y = H / 2 + sa * u + ca * v + (r() - .5) * jitter * cell;
...
function paper(c, base = PAL.paper, band = PAL.paperBand, seed = 5) {
  resetT(c); c.fillStyle = base; c.fillRect(0, 0, c.canvas.width / S, c.canvas.height / S);
...
function aster(c, x, y, r, n, color, seed, g = 1, core = PAL.night, rim = PAL.chalk) {
...
function speedLines(c, x, y, dir, seed, n = 7, al = .6, color = PAL.ink) {
  const r = rng(seed); c.save(); c.strokeStyle = color; c.globalAlpha = al; c.lineCap = 'round';
  for (let i = 0; i < n; i++) {
    const side = (r() - .5) * 70, back = 30 + r() * 40, L = 60 + r() * 120, px = -Math.sin(dir), py = Math.cos(dir), x0 = x - Math.cos(dir) * back + px * side, y0 = y - Math.sin(dir) * back + py * side;
    c.lineWidth = .8 + r() * 1.8; c.beginPath(); c.moveTo(x0, y0); c.lineTo(x0 - Math.cos(dir) * L, y0 - Math.sin(dir) * L); c.stroke();
  } c.restore();
}
function loops(c, x, y, dir, seed, al = .6, cols = PAL.accents) {
  const r = rng(seed); c.save(); c.translate(x, y); c.rotate(dir); c.globalAlpha = al; c.lineWidth = 1.2;
...
function plant(c, x, y, len, depth, seed, o = {}) {
  const { stem = PAL.ink, leaf = PAL.accents[0], flower = PAL.accents[1], angle = -Math.PI / 2, width = 1.3 } = o; const r = rng(seed); c.save();
  (function branch(x, y, len, a, d, w) {
    const x2 = x + Math.cos(a) * len, y2 = y + Math.sin(a) * len; c.strokeStyle = stem; c.lineWidth = w; wob(c, [[x, y], [(x + x2) / 2 + (r() - .5) * len * .2, (y + y2) / 2 + (r() - .5) * len * .2], [x2, y2]], 1, seed + d * 13 + (r() * 100 | 0));
    if (d <= 0) {
      c.fillStyle = leaf; c.globalAlpha = .7; for (let k = 0; k < 5; k++) { const la = a + (r() - .5) * 2.2; c.beginPath(); c.ellipse(x2 + Math.cos(la) * 6, y2 + Math.sin(la) * 6, 7, 3, la, 0, TAU); c.fill(); } c.globalAlpha = 1;
    ...
    const n = 2 + (r() < .5 ? 1 : 0); for (let k = 0; k < n; k++) branch(x2, y2, len * (.55 + r() * .25), a + (r() - .5) * 1.4, d - 1, w * .75);
  })(x, y, len, angle, depth, width); c.restore();
}
...
function signOff(c, a, b, o = {}) {
  const { x = 540, y = 540, size = 60, ink = PAL.ink, ink2 = PAL.accents[0], progressA = 1, progressB = 1 } = o; const ta = a.slice(0, Math.round(a.length * progressA)), tb = b.slice(0, Math.round(b.length * progressB));
...
function blot(c, src, cx, cy, R, seed, fringe = PAL.night) {
  if (R <= 0) return; const r = rng(seed), path = new Path2D(); for (let i = 0; i < 72; i++) { const a = i / 72 * TAU, rr = R * (1 + (r() - .5) * .16), x = cx + Math.cos(a) * rr, y = cy + Math.sin(a) * rr; i ? path.lineTo(x, y) : path.moveTo(x, y); } path.closePath();
...
function mosaic(c, src, s, box = [0, 0, W, H], tintCol = 'rgba(120,160,255,.10)') {
  const ow = src.width, oh = src.height, img = src.getContext('2d').getImageData(0, 0, ow, oh).data; c.save(); resetT(c);
...
function badges(c, cards, o = {}) {
  const { cx = 540, cy = 540, r0 = 40, gap = 118, size = 52, ring = PAL.ink, seed = 1, progress = 1, count = cards.length, scale = 1 } = o;
  ...
  c.save(); resetT(c); c.translate(cx, cy); c.scale(scale, scale); c.translate(-cx, -cy); let placed = 0, k = 1; while (placed < count && k < 8) {
    const R = r0 + k * gap, n = Math.min(count - placed, Math.max(5, Math.round(TAU * R / (size * 2.5)))); dashedRing(c, cx, cy, R, ring, seed + k);
...
function registerPhoto(name, meta) {
  const img = new Image(), ph = PHOTOS[name] = { ...meta, name, img };
...
function place(name, { x = CX, y = CY, h, w, rot = 0, flip = false, pivot = null } = {}) {
  const ph = PHOTOS[name]; if (!ph) throw new Error('unknown photo: ' + name + ' (is photos.js loaded after core.js?)');
...
function photo(c, pl, o = {}) {
  let { shadow = .3, shadowW = .92, al = 1, clip = null, ground = null } = o; c.save(); if (_chalk) { shadow = 0; al = 1; c.globalCompositeOperation = 'destination-out'; }   // chalk pass: the object erases the chalk behind it   // ground = y of the floor, when the object lifts off it
  if (shadow && !clip) {
    const q = [[0, 0], [1, 0], [1, 1], [0, 1]].map(([u, v]) => on(pl, u, v)), xs = q.map(p => p[0]), ys = q.map(p => p[1]), by = ground ?? Math.max(...ys), mx = (Math.min(...xs) + Math.max(...xs)) / 2, rx = (Math.max(...xs) - Math.min(...xs)) / 2 * shadowW, ry = Math.max(10, rx * .13);
...
function photoSheet(c, pl) {
  backdrop(c); photo(c, pl); c.save(); c.font = '15px ui-monospace, Menlo, monospace'; c.lineWidth = 1;
  for (let k = 0; k <= 10; k++) {
    const u = k / 10; c.strokeStyle = k % 5 ? 'rgba(0,80,255,.35)' : 'rgba(0,80,255,.85)'; c.beginPath(); c.moveTo(...on(pl, u, 0)); c.lineTo(...on(pl, u, 1)); c.moveTo(...on(pl, 0, u)); c.lineTo(...on(pl, 1, u)); c.stroke();
...
function backdrop(c, base = PAL.paper, o = {}) {
  if (_chalk) { viewT(c); return; } const { vignette = .16, spot = .3, seed = 5 } = o; paper(c, base, null, seed); c.save(); resetT(c); const R = Math.hypot(W, H) / 2;
...
function nightShot(c, body, o = {}) {
  const { k = .86, lights = [], color = PAL.night } = o; body(c); if (k <= 0) return;
  ...
  viewT(c); for (const l of lights) if (l.glow !== 0 && l.r > 0) { glow(c, l.x, l.y, l.r * .55, l.color || '#ffcf70', (l.glow ?? .16) * k); glow(c, l.x, l.y, Math.min(90, l.r * .22), l.color || '#ffcf70', Math.min(1, (l.glow ?? .16) * 4) * k); }
}   // a faint warm air and a tight halo at the source
...
function _profile(ph) {
  if (ph.prof) return ph.prof; const n = 240, cvs = document.createElement('canvas'); cvs.width = cvs.height = n; const g = cvs.getContext('2d', { willReadFrequently: true }); g.drawImage(ph.img, 0, 0, n, n); const d = g.getImageData(0, 0, n, n).data, A = (x, y) => d[(y * n + x) * 4 + 3] > 96;
...
function spline(pts, step = 5, close = false) {
  if (pts.length < 3) { const [a, b] = pts, n = Math.max(1, Math.round(Math.hypot(b[0] - a[0], b[1] - a[1]) / step)); return Array.from({ length: n + 1 }, (_, k) => [lerp(a[0], b[0], k / n), lerp(a[1], b[1], k / n)]); }
  const P = i => close ? pts[(i + pts.length) % pts.length] : pts[clamp(i, 0, pts.length - 1)], out = [], segs = close ? pts.length : pts.length - 1;
  for (let i = 0; i < segs; i++) {
    const p0 = P(i - 1), p1 = P(i), p2 = P(i + 1), p3 = P(i + 2), n = Math.max(2, Math.round(Math.hypot(p2[0] - p1[0], p2[1] - p1[1]) / step));
...
function brush(c, pts, o = {}) {
  const { w = 4, color = PAL.ink, p = 1, seed = 1, amp = 1.6, taper = 1, close = false, smooth = true, al = 1 } = o; if (p <= 0 || pts.length < 2) return;
  ...
  let a = pt(0); for (let i = 1; i < q.length && s[i - 1] < end; i++) {
    let b = pt(i); if (s[i] > end) { const t = (end - s[i - 1]) / (s[i] - s[i - 1]); b = [lerp(a[0], b[0], t), lerp(a[1], b[1], t)]; }
...
function wash(c, path, color, o = {}) {
  let { al = .5, off = 5, seed = 1, blend = PAL.washBlend || 'multiply', rim = true } = o; if (_chalk && !o.blend) al *= .6;   // colour is dimmer in the dark, unless the wash is a light itself
...
function doodle(o = {}) {
  const { start = 0, speed = 1000, gap = .03, seed = 1, w = 4, color } = o; const ops = []; let t = start, z = 0, n = 0;
  const api = {
    at(time) { t = time; return api; }, wait(d) { t += d; return api; }, layer() { z++; return api; }, get end() { return t; }, get ops() { return ops; },
    ...
    draw(c, tau, extra = 0) {
      for (let zz = 0; zz <= z; zz++) for (const pass of [0, 1, 2]) for (const op of ops) {
        if (op.z !== zz) continue; const k = clamp((tau - op.t0) / op.dur, 0, 1); if (k <= 0) continue;
        ...
        else if (pass === 2 && op.kind === 'mark') op.fn(c, k);
      } return api;
    }
  };
  return api;
...
function styleSheet(c, tau = 0, i = 0) {
  paper(c); c.font = '16px ui-monospace, Menlo, monospace';
...
function paletteSheet(c) {
  paper(c); c.font = '18px ui-monospace, Menlo, monospace'; const rows = [['paper', [PAL.paper]], ['ink', [PAL.ink]], ['night', [PAL.night]], ['chalk', [PAL.chalk, PAL.chalkDim]], ['fills', PAL.fills], ['shade/light/blush', [PAL.shade, PAL.light, PAL.blush]], ['accents', PAL.accents], ['inks', PAL.inks]];
...
function gridSheet(n = 24, cellW = 240, startFrame = null) {
  ...
  for (let k = 0; k < n; k++) {
    const i = startFrame === null ? Math.round(k * (FILM.NDRAW - 1) / Math.max(1, n - 1)) : clamp(startFrame + k, 0, FILM.NDRAW - 1); cur = -1; show(i);
...
function locate(i) {
  if (!Number.isInteger(i) || i < 0 || i >= FILM.NDRAW) throw new RangeError('Frame outside film: ' + i); const { timeline } = FILM; const t = i / FPS_DRAW; let acc = 0;
...
function buildPlayer() {
  ...
  bar.querySelector('#exp').onclick = async () => {
    if (!window.showDirectoryPicker) { ui.msg.textContent = 'no File System Access API here: use render.mjs'; return; } const dir = await showDirectoryPicker({ mode: 'readwrite' }); playing = false;
