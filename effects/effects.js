...
<html lang="en">
  <head>
    ...
    <style>
      ...
      /* Container to hold all canvases and catch pointer events for OrbitControls */
      #canvasContainer {
        position: absolute;
        top: 0;
        left: 0;
  ...
  <body>
    <!-- Axis HUD (Only compass) -->
    <div id="axisHUD">
      <canvas id="axisCanvas" width="100" height="100"></canvas>
    </div>

    <!-- Floating toggle for when sidebar is hidden -->
    <div id="floatingToggle">+</div>

    <!-- Container for all canvases (handles unified orbit controls) -->
    ...
    <div id="sidebar">
      ...
      <div id="modelUI">
        ...
        <select id="modelSelect">
          ...
          <option value="horse">Horse (OBJ)</option>
          <option value="tableLamp">Table Lamp</option>
          <option value="chair">Chair</option>
          <option value="lantern">Lantern</option>
          <option value="olives">Olives</option>
          <option value="sofa">GlamVelvetSofa</option>
          <option value="cesiumMan">Cesium Man</option>
          <option value="milkTruck">Milk Truck</option>
          <!-- <option value="head">Head</option> -->
          <option value="dragon">Dragon</option>
          <option value="hands">Hands (Creation of Adam)</option>
          <option value="duck">Duck</option>
          <option value="avocado">Avocado</option>
          <option value="lamp">Lamp</option>
          <option value="skull">Scattering Skull</option>
          <option value="hand">Hand</option>
          <option value="astronaut">Astronaut (OBJ)</option>
    ...
    <script type="module">
      ...
      const complexShader = `
        ...
        void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
          ...
          else if (luma < 0.35) pattern = cross(cellUV, 0.12);
          else if (luma < 0.55) pattern = square(cellUV, 0.30);
          else if (luma < 0.75) pattern = circle(cellUV, 0.28);
          else pattern = circle(cellUV, 0.12);

          float edgeBoost = smoothstep(0.0, 0.2, 1.0 - luma);
          vec3 blue = vec3(0.0, 0.31, 0.93);
          vec3 finalColor = mix(bg, blue, pattern * (0.6 + edgeBoost * 0.4));
      ...
      const noise_shader = `
        ...
        void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
          ...
          outputColor = vec4(vec3(clamp(s,0.0,1.0))*mask, 1.0);
        }
      `;
      ...
      const vhsShader = `
        ...
        void mainImage(const in vec4 i, const in vec2 uv, out vec4 o) {
          ...
          float shift = 0.005;
          float r = texture2D(inputBuffer, u + vec2(shift, 0.0)).r;
          float g = texture2D(inputBuffer, u).g;
          float b = texture2D(inputBuffer, u - vec2(shift, 0.0)).b;
      ...
      class MatrixRainEffect {
        constructor(renderer, camera) {
          ...
          this.domElement.style.pointerEvents = "none"; // Let interactions pass through if needed
      ...
      /* ================= CONTROLS ================= */
      // Controls attached to PARENT CONTAINER so rotation works everywhere!
      ...
      const MODELS = {
        ...
        skull: {
          url: "https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Assets/main/Models/ScatteringSkull/glTF-Binary/ScatteringSkull.glb",
          scale: 8.0,
        ...
        head: {
          url: "https://raw.githubusercontent.com/pichiliani/ModelsOBJ/master/head.obj",
          scale: 0.1,
          rotation: { x: 0.3, y: Math.PI / 2, z: 0 },
          position: { x: 0, y: -0.7, z: 0 },
        },
        milkTruck: {
          url: "https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/main/2.0/CesiumMilkTruck/glTF-Binary/CesiumMilkTruck.glb",
          scale: 0.5,
        ...
        sofa: {
          url: "https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/main/2.0/GlamVelvetSofa/glTF-Binary/GlamVelvetSofa.glb",
          scale: 1.5,
          rotation: { x: 0, y: Math.PI, z: 0 },
          position: { x: 0, y: -0.7, z: 0 },
        },
      ...
      function loadModel(name) {
        ...
        loader.load(
          ...
          (result) => {
            ...
            // Reset camera target internally but do NOT recreate OrbitControls
            controls.target.copy(handsFocusPoint);
            controls.update();
            camera.lookAt(handsFocusPoint);
          },
      ...
      //locking the horizonal rotation only (nopt allowing the model to rotate in vertical direction)
      ...
      function updateAxisHUD() {
        ...
        // Use inverse camera quaternion so gizmo moves when you orbit (real-time)
      ...
      pixelInput.oninput = (e) => {
        ...
        ["dots", "halftone", "blockify", "complex", "woven", "lego"].forEach(
          (key) => {
      ...
      function animate() {
        ...
        // Updates orbit controls attached to container
        controls.update();

        // smooth width animation when sidebar toggles
