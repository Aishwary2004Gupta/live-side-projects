...
const wovenShader = `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    vec2 noisyCenter = centered + (vec2(random(cellPos + centered), random(cellPos + centered)) - 0.5) * noiseAmount;
    ...
    // Enhanced weave pattern visibility
    color.rgb *= smoothstep(0.15, 1.0, 1.0-ellipse) * 1.2;
    
    float stripeNoise = noise(vec2(centered.x, centered.y * 100.0));
    color.rgb *= stripeNoise + 0.5;
    
    float hueShift = (random(cellPos)-0.5)*0.08;
    vec3 hsv = rgbToHsv(color.rgb);
    hsv.x += hueShift;
    hsv.y *= 1.1; // Slight saturation boost for better color pop
...
const MODELS = {
    ...
    try: {
        url: "https://raw.githubusercontent.com/Aishwary2004Gupta/models/cloud/guitar_inUse.glb",
        scale: 2,
        rotation: { x: -1.3, y: Math.PI, z: 1.5 },
        position: { x: 0, y: 0, z: 0 },
    },
};
