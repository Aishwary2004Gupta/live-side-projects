...
export const voronoiShader = `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    // Keep a very slow drift only, instead of fast moving points
    vec2 drift = vec2(time * 0.015, time * 0.01);
    ...
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        ...
        // Stable random point per cell.
        // Removed time-based animation to stop sparkling.
        vec2 point = vec2(0.5) + 0.28 * hash(cellPos);
    ...
    // Filled region mask
    float border = smoothstep(0.015, 0.09, borderDist);

    // Softer, more stable white outline
    float outline = 1.0 - smoothstep(0.0, 0.018, borderDist);
    outline *= 0.75;
    ...
    // Add calmer white Voronoi outline
