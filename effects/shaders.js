...
export const voronoiShader = `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    // Main filled region mask
    float border = smoothstep(0.01, 0.08, borderDist);

    // Thin white outline mask
    float outline = 1.0 - smoothstep(0.0, 0.025, borderDist);

    // Keep pure background black
    if (l < 0.02) {
    ...
    // Add white Voronoi outlines on top
